...
export default function Expense() {
  ...
  const { expense, getExpenses, deleteExpenses } = useGlobalContext();
  useEffect(() => {
  ...
  return (
    ...
    <div className="w-1/2"><h1 className='text-4xl font-bold text-[#07271f]' style={{fontFamily:'Kaushan Script'}}>Expenses</h1></div>
    <div className="w-1/2 flex justify-end items-center">
<button className="btn bg-[#07271f] rounded-btn text-lg text-white" onClick={()=>document.getElementById('my_modal_5').showModal()}>Add Expense</button>
...
<select value={category} className='px-4 py-2 rounded-md' onChange={(e) => setCategory(e.target.value)}>
<option value="" disabled >Select Option</option>
      ...
      <select value={sortBy} className='px-4 py-2 rounded-md' onChange={(e) => setSortBy(e.target.value)}>
        {/* Options for selecting sorting */}
        <option value="createdAt-dsc" disabled >Select Option</option>
  <option value="amount_asc">Lowest to Highest (Amount)</option>
  <option value="amount_desc">Highest to Lowest (Amount)</option>
      </select>
<div className="">
        <button className=' rounded-md px-4 py-2 bg-[#07271f] text-white' onClick={handleReset}>reset</button>
      </div>
</div>
    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        {expense.length === 0 ? (
        <p className="flex justify-center items-center w-full py-10 text-xl text-gray-500">No expenses to display</p>
        ):(
        expense.map((expense) => {
